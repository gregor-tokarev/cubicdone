...
import { github, google, linear } from "../auth/providers";
import { db } from "../db";
...
async function defaultResponse(
    ...
    res.cookie("session", session.id, {
        ...
        domain: process.env.NODE_ENV === "production" ? "cubicdone.com" : "localhost",
        sameSite: "lax",
    });

    return res.redirect(process.env.NODE_ENV === "production" ? "https://app.cubicdone.com" : "http://localhost:3000");
}
...
oauthRedirectRouter.get("/github", async (req, res) => {
    const code = req.query["code"];
