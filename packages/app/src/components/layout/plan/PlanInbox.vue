...
function toggleSelection() {
    ...
    if (!props.selectedDraftIds.includes(hoveredDraftId.value)) {
        emit("update:selectedDraftIds", [...props.selectedDraftIds, hoveredDraftId.value])
    } else {
...
<template>
    <div class="space-y-2.5">
        <div class="flex items-center space-x-5">
            ...
            <div v-if="loadingDrafts" class="flex items-center space-x-1.5 text-gray-600">
                <VueSpinner size="16px"></VueSpinner>
                <span>{{ t("loading") }}</span>
            </div>
        </div>
        <VueDraggableNext class="flex min-h-[78px] items-center space-x-2.5 overflow-auto pb-2.5" :list="allDrafts"
            @mouseleave="onListLeave" @change="onChangeDraft($event)" @start="onStartDraggingDraft($event)"
            :group="{ name: 'tasks', pull: 'clone' }">
            <DraftCard class="min-w-[25%] grow" :hovered="hoveredDraftId === d.id"
                :selected="selectedDraftIds.includes(d.id)" :class="{
                    'max-w-[25%]': allDrafts.length >= 4,
                    'max-w-[33%]': allDrafts.length === 3,
                    'max-w-[50%]': allDrafts.length === 2,
                }" v-for="d in allDrafts" :key="d.id" :data-id="d.id" @contextmenu.prevent="onContexMenuOpen"
                @mouseenter="hoveredDraftId = d.id" :draft="d">
            </DraftCard>
            <div v-if="!allDrafts.length"
