...
<template>
    <div ref="root" class="flex flex-col rounded-lg bg-gray-50 px-2.5 py-1.5" :data-date="date">
        ...
        <VueDraggableNext class="grow space-y-2.5 overflow-y-auto overflow-x-hidden"
            ...
            :group="{ name: 'tasks' }">
            <TaskCard v-for="t in taskStore.getByDate(date.toString())" :key="t.id" :task="t"
                :hovered="t.id === hoveredTask" :selected="selectedIds.includes(t.id)"
                @mouseenter="emit('hoverTask', t.id)" data-drag :data-id="t.id"
