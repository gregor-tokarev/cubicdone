...
<template>
    <div class="cursor-grab space-y-1 rounded-lg bg-gray-100 px-4 py-2.5 transition-colors hover:bg-gray-50 active:cursor-grabbing border-2 border-transparent"
        :class="{ 'bg-gray-50': hovered, '!border-black': selected }">
        <Markdown :model-value="draft.title"></Markdown>
        <div class="flex items-center space-x-1">
            <ProjectTag v-if="draft.projectId" :project-id="draft.projectId"></ProjectTag>
            <a v-if="draft.external"
                class="flex cursor-pointer items-center space-x-1 rounded-lg bg-gray-400 px-1.5 py-1"
                :href="draft.external.link" target="_blank">
