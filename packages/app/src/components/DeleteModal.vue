...
<template>
    <teleport to="[data-scroll-container]">
        <div v-if="deleteModalStore.open && deleteModalStore.modalOptions !== null"
            ...
            <div v-if="deleteModalStore.resolveFn" class="flex justify-end space-x-2">
                <BaseButton ref="cancelButton" @click="onButtonClick(false)" color="gray">{{ t("cancel") }}</BaseButton>
                <BaseButton class="shadow-gray-900 focus:shadow-2xl" ref="submitButton" @click="onButtonClick(true)">
                    {{ t("delete") }}</BaseButton>
            </div>
        </div>
    </teleport>
    <teleport to="body">
        <div v-if="deleteModalStore.open" @click="deleteModalStore.close()" class="fixed inset-0"></div>
    </teleport>
</template>
