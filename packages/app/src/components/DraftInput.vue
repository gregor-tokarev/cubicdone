...
function handleProject(evt: KeyboardEvent) {
    ...
    setTimeout(() => {
        if (evt.code !== "ArrowUp" && evt.code !== "ArrowDown" && !evt.ctrlKey)
    ...
    } else if (evt.code === "ArrowUp" || evt.code === "ArrowDown" || ((evt.ctrlKey && evt.code === "KeyN") || (evt.ctrlKey && evt.code === "KeyP"))) {
...
<template>
    <div class="relative" ref="root">
        ...
        <div class="flex items-center rounded-lg bg-gray-100 px-2.5 py-1">
            ...
            <div class="flex grow items-center space-x-1" @keydown="onUpdate($event)" ref="partsContainer">
                <template v-for="(part, idx) in modelValue" :key="part.id">
                    <Contenteditable class="outline-0" tag="p" :class="{
                        grow: modelValue.length - 1 === idx || modelValue.length === 0,
                    }" v-if="part.type === 'text'" @focus="onFocus($event)" v-model="part.content">
                    </Contenteditable>
                    <Contenteditable class="rounded-lg bg-gray-300 px-[3px] py-[2.5px] leading-[120%] outline-0" :class="{
                        [`!bg-${'projectId' in part &&
                            part.projectId &&
                            projectStore.getOne(part.projectId)?.color
                            }-100`]: 'projectId' in part && part.projectId,
                        [`!text-${'projectId' in part &&
                            part.projectId &&
                            projectStore.getOne(part.projectId)?.color
                            }-400`]: 'projectId' in part && part.projectId,
                    }" tag="p" v-else-if="part.type === 'project'" v-model="part.content" @focus="onFocus($event)">
                    </Contenteditable>
                </template>
            </div>
        </div>
        <!--    Project select-->
        <teleport to="body">
            <div v-if="openProjectSearch && projectSelectBound"
                class="absolute left-10 top-10 rounded-lg bg-gray-100 py-1.5" :style="{
                ...
                <template v-if="projectSelectOptions">
                    <SelectModalOption v-for="(p, idx) in projectSelectOptions" :active="projectOptionSelected === idx"
                        :option="{ id: p.id, text: p.title, color: p.color }" :check="false"
                        @click="onClickProject(p.id)" />
                </template>
                <div v-if="projectQuery && !queryMatchProject" class="flex items-center space-x-1.5 px-2.5 py-1.5"
                    :class="{
                        'bg-gray-300':
