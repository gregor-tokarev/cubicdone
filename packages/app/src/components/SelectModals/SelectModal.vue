...
<template>
    <teleport to="[data-scroll-container]">
        <div v-if="open"
            ...
            <input type="text" ref="searchEl" :value="query" @input="onInput" @keydown="onKeydown"
                class="border-gray-150 w-full border-b bg-transparent p-2 placeholder-gray-500 outline-0"
                :placeholder="t('placeholder')" />
            <div v-if="options" class="">
                <ProjectOption v-for="(p, idx) in options" :option="p" :check="idx === checkedIndex"
                    :active="selectedProjectIdx === idx" @click="onClick(p.id)"></ProjectOption>
            </div>
        </div>
    </teleport>
    <teleport to="body">
        <div v-if="open" @click="emit('update:open', false)" class="fixed inset-0"></div>
    </teleport>
</template>
