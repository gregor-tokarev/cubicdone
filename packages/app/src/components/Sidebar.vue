...
const filteredOptions = computed(() => {
    ...
    return localeQuery.value ? fuse.search(localeQuery.value).map(r => r.item) : localeOptions
})
...
<template>
    <div class="flex w-[250px] flex-col rounded-br-2xl rounded-tr-2xl bg-black px-[18px] py-4"
        ...
        <div class="flex cursor-pointer items-center space-x-2 rounded px-1.5 py-2 text-base mt-auto text-gray-200 transition-colors hover:text-white hover:bg-gradient-to-r from-[#1A1A1A] to-[#141414]"
            :class="{
                'h-unset w-full': !compact,
                'h-[32px] w-[32px] justify-center': compact,
