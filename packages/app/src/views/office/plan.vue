...
<template>
    <div class="flex h-[100vh] flex-col space-y-4 py-8">
        <!--  Inbox section-->
        <PlanInbox v-model:integration-drafts="integrationDrafts" v-model:selected-draft-ids="selectedDraftIds"
            :activated-integrations="activatedIntegrations">
        </PlanInbox>
        <!--    Date columns-->
        <PlanPlanner class="grow" v-model:integration-drafts="integrationDrafts"
            v-model:selected-task-ids="selectedTaskIds"></PlanPlanner>
    </div>
    <CommandPalette ref="commandEl" class="fixed bottom-0 left-1/2 -translate-x-1/2 translate-y-full"
        :selected-count="totalCount" :commands="[
            { name: t('setProject'), icon: 'hashtag', emitName: 'setProject' },
            { name: t('delete'), icon: 'basket', emitName: 'remove' },
        ]" @discard="onDiscard" @remove="onDelete" @setProject="onSetProject"></CommandPalette>
</template>
