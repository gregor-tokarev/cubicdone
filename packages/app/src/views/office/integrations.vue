...
import { useRouter } from "vue-router"; import ErrorMessage from "@components/ErrorMessage.vue";
...
<template>
    <div class="pt-8">
        ...
        <div class="grid grid-cols-2 gap-5">
            <IntegrationCard v-for="(i, idx) in integrationStore.mappedIntegrations" :integration="i" :key="idx"
                @connect="onConnect(i.id)" @disconnect="onDisconnect(i.id)"
                @link="router.push(`/integrations/mapper/${i.name}`)"></IntegrationCard>
        </div>
    </div>
    <teleport to="body">
        <BaseModal v-if="openIntegrationId && openIntegration" @cancel="openIntegrationId = ''">
            <div class="w-[670px]">
                <div class="mb-6 flex items-center space-x-2">
                    <img :src="openIntegration.iconURL" class="!h-6 !w-6 object-contain" :alt="openIntegration.name" />
                    <h2 class="text-xl">{{ openIntegration.name }}</h2>
                </div>
                <div class="mb-9 space-y-3">
                    <ApiKeyCard v-for="apiKey in openIntegration.apiKeys" :api-key="apiKey" :key="apiKey.id"
                        @revoke="onRevoke(apiKey.id)"></ApiKeyCard>
                </div>
                <div class="mb-5 space-y-3">
                    <div class="space-y-0.5">
                        <div class="flex items-center space-x-1">
                            <p class="text-xs capitalize text-gray-500">{{ t("modal.label") }}</p>
                            <p v-if="v$.label.$errors[0]" class="text-xs text-red-400">
                                {{ v$.label.$errors[0].$message }}
                            </p>
                        </div>
                        <BaseInput v-model="v$.label.$model" :error="v$.label.$error" :autocomplete="false"
                            @blur="v$.label.$touch" type="text" placeholder="your_workspace"></BaseInput>
                    </div>
                    <div class="space-y-0.5">
                        <div class="flex items-center space-x-1">
                        ...
                        <BaseInput v-model="v$.key.$model" type="password" :autocomplete="false" :error="v$.key.$error"
                            @blur="v$.key.$touch" placeholder="lin_api_k7Yk0QrBjjdTyzBAAHiW1SyTR23ycZoZHu3eHfGU">
                        </BaseInput>
                    </div>
